...
function createPlan(amount, girlName) {
  return new Promise(async (resolve, reject) => {
    try {
      const product = await stripe.products.create({
        name: `Ocultuz - Subscripción mensual ${girlName}`,
      });
  ...
  mercadopago
    ...
    .then((response) => {
      const paymentMethods = response.body.filter((paymentMethod) => {
        return (
          (paymentMethod.payment_type_id == "ticket" ||
            paymentMethod.payment_type_id == "atm") &&
...
function createPendingSub(body, subscription) {
...
app.post("/store", mdAuth, (req, res) => {
  ...
  mercadopago.payment
    ...
    .then((charge) => {
      const date_created = charge?.response?.date_created
        ? charge.response.date_created
        : new Date();
      ...
      purchase.save(async (err, purchaseSaved) => {
        ...
        if (body.type === "subscription") {
          await createPendingSub(req.body, charge.body);
        }
          ...
          .then((info) => {
            return res.status(200).json({
              ...
              message:
                ...
                req.user.email +
                " sigue los pasos para continuar la compra!",
            });
          })
          .catch((error) => {
            return res.status(500).json({
