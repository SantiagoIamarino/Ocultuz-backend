...
app.get("/", (req, res) => {
  Girl.find(
    ...
    "_id previewImage nickname status subscriptionPrice",
    (err, girls) => {
      if (err) {
...
function validateContent(content, user) {
  return new Promise((resolve, reject) => {
    ...
    if (user._id != content.girlId) {
      // Allow if same girl is requesting
      Purchase.findOne(
        {
        ...
        (err, contentDB) => {
          ...
          if (!contentDB || contentDB.pending) {
            // Don't show content to user
            content.fileUrl = "";
...
function validateSub(subscriptionDB) {
  return !(!subscriptionDB || !subscriptionDB.active);
}

const retrieveBasicContent = async (girlId, perPage, page) => {
  const contentsDB = await Content.find({
    girlId,
    type: "general",
  })
  ...
  const basicTotal = await Content.count({
    girlId,
    type: "general",
  });
...
app.post("/get-basic-content/:girlId", mdAuth, async (req, res) => {
  const girlId = req.params.girlId;
  const perPage = req.body.perPage;
  const page = req.body.page;

  try {
    if (req.user._id === girlId) {
      // Same user (content owner)
      const { basicContent, basicTotal } = await retrieveBasicContent(
        girlId,
        perPage,
        page
      );
    ...
    //Another user, not content owner
...
app.get("/profile/:girlNickname", mdAuth, async (req, res) => {
  const girlNickname = req.params.girlNickname;

  let contentToRetrieve =
    "_id nickname description status banner previewImage identityVerified tips paypalAccount";

  if (
    (req.user?.nickname && girlNickname === req.user.nickname) ||
    req.user?.role === "ADMIN_ROLE"
  ) {
    contentToRetrieve += " basicContent products";
  }

  try {
    const girlDB = await Girl.findOne(
...
app.put("/:girlId", [mdAuth, mdSameUser], (req, res) => {
  ...
  Girl.findById(girlId, async (err, girlDB) => {
    ...
    if (
      girlDB.previewImage &&
      girl.previewImage &&
      girlDB.previewImage !== girl.previewImage
    ) {
...
app.delete("/:girlId", [mdAuth, mdSameUser], (req, res) => {
  ...
  Girl.findById(girlId, async (err, girlDB) => {
    ...
    girlDB.delete((errDlt, girlDeleted) => {
      ...
      Verification.findOneAndDelete(
        { girl: girlDB._id },
        (errDelVer, verificationDeleted) => {
          return res.status(200).json({
            ok: true,
