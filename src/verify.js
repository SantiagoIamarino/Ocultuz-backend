...
async function run() {
  ...
  try {
    ...
    console.log(
      ...
      "dbNextPayment",
      " - ",
      "(subNextPayment > dbNextPayment)"
    );

    const subscriptionsFoundPromises = [];
    await subscriptionsDB.forEach(async (subscription) => {
      subscriptionsFoundPromises.push(
        verifySubscription(subscription, database)
      );
    });
...
async function updateSub(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    subscriptionEnds.setDate(
      subscriptionEnds.getDate() + config.daysBeforeCancell
    );
...
function verifySubscription(subscription, database) {
  ...
  url += `?access_token=${mpAccessToken}`;
    ...
    axios
      ...
      .then((response) => {
        if (response.data.results.length == 0) {
          console.log(
            "Sub not found on MP - ",
            subscription.paymentData.preapproval_plan_id
          );
          resolve();
        }
        ...
        if (subNextPayment > dbNextPayment) {
          updateSub(subscription, subNextPayment, lastCharge, database)
            .then(() => {
